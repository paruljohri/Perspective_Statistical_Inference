...
#python mask_exonic_regions.py full/path/to/eqm_dfe_exon50_100kb_var_rates_masked repID exon50 msprime

import sys
folder = sys.argv[1]
rep_num = sys.argv[2]#1/2/3...100
genome = sys.argv[3]#exon20/exon50
sim_type = sys.argv[4]#slim/msprime

#Read the genome coordinates and figure out which sites to mask:
if genome == "exon20":
    f_genome = open("/home/pjohri1/ModelRejection/genome_structure_exon20.txt", 'r')
    if sim_type == "slim":
        tot_len = 97625
    elif sim_type == "msprime":
        tot_len = 97626
elif genome == "exon50":
    f_genome = open("/home/pjohri1/ModelRejection/genome_structure_exon50.txt", 'r')
    if sim_type == "msprime":
        tot_len = 99012
    elif sim_type == "slim":
        tot_len = 99012-1
