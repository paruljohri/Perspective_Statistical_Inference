...
parser.add_argument('-mutnTypes', dest = 'mutnTypes', action='store', nargs = 1, type = str, help = 'm1,m2,m3')
...
result.write("filename" + '\t' + "num_of_substitutions" + '\t' + "num_fixed_sites_sample" + '\t' + "div_per_site" + '\n')
...
for Aline in f_list:
    ...
    num_substitutions = avg_divergence_win(d_subs, 0.0, 1.0)
    f_subs.close()
    ...
    #Write both numbers:
    result.write(f_name + '\t' + str(num_substitutions) + '\t' + str(num_fixed_sites) + '\t' + str(float(num_substitutions+num_fixed_sites)/float(masked_chr_len)) + '\n')
