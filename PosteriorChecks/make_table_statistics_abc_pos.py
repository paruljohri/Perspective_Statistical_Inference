...
l_scenarios = ["scenario2", "scenario5", "scenario10", "scenario18", "scenario19", "scenario26", "scenario27", "scenario29", "scenario30", "scenario31", "scenario32", "scenario33", "scenario34", "scenario35", "scenario36", "scenario37", "scenario38"]
win_size = 2000 #2000 or 5000 or 10000
tot_coding_sites=300*5*33 #exon length * num of exons per gene * num of genes
l_stats = ["S", "thetapi", "thetaw", "thetah", "hprime", "tajimasd", "numSing", "hapdiv", "rsq", "D", "Dprime", "divergence", "lambda", "mu_ben"]
...
def get_mean_within_reps(d_STATS):
    ...
    for stat in l_stats[0:len(l_stats)]:
        if stat!="divergence" and stat!="lambda" and stat!="mu_ben":#these stats are calcualted for the whole chromosome
            d_mean_STATS[stat] = []
            for s_rep in l_reps:
...
#Result:
result = open("/home/pjohri1/ModelRejection/posterior_checks_pos_" + str(win_size) + ".stats", 'w+')
result.write(d_par["scenarioID"])
for x in l_stats:
...
for simID in l_scenarios:
    print (str(simID))
    try:
        if os.stat("/scratch/pjohri1/ModelRejection/PosteriorChecks/eqm_pos_selection_stats/" + str(simID) + "_" + str(win_size) + ".stats").st_size > 0:
            f_stats = open("/scratch/pjohri1/ModelRejection/PosteriorChecks/eqm_pos_selection_stats/" + str(simID) + "_" + str(win_size) + ".stats", 'r')
            check = 1
        else:
            print ("Empty file")
            check = 0
    ...
    if check == 1:#stats file has been read
        ...
        if len(d_stats["filename"]) == 100:#all 100 replicates are present
            ...
            #if simID > 0:
            try:
                f_div = open("/scratch/pjohri1/ModelRejection/PosteriorChecks/eqm_pos_selection_stats/" + str(simID) + ".divergence", 'r')
            ...
            f_div_l = open("/scratch/pjohri1/ModelRejection/PosteriorChecks/eqm_pos_selection_stats/" + str(simID) + "_lambda.divergence", 'r')
            d_mean_stats["lambda"] = []
            d_mean_stats["mu_ben"] = []
            for line in f_div_l:
                line1 = line.strip('\n')
                ...
                if "num" not in line:
                    ...
                    d_mean_stats["mu_ben"].append(float(line2[1])/float(tot_coding_sites))
            f_div_l.close()
            #write it
            result.write(d_par[str(simID)])
            for stat in l_stats:
